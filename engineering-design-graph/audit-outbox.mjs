...
export function createAuditOutboxStore(){
  ...
  function record({principal,method,path,status,requestId,body,replayed=false}){
    const at=now(),actorUserId=principal?.userId||null,t=target(path);
    const audit={id:randomUUID(),actorUserId,action:`${method} ${path}`,targetType:t.type,targetId:t.id,status,requestId,replayed,metadata:{bodyKeys:body&&typeof body==='object'?Object.keys(body):[]},createdAt:at};
    auditLogs.push(audit);
    if(!replayed&&method!=='GET'&&method!=='HEAD'&&status>=200&&status<300){outboxEvents.push({id:randomUUID(),topic:classify(method,path),aggregateType:t.type,aggregateId:t.id||requestId,payload:{actorUserId,method,path,status,auditLogId:audit.id},status:'pending',createdAt:at,publishedAt:null});}
    return audit;
  }
...
export class PostgresAuditOutboxStore{
  ...
  async record({principal,method,path,status,requestId,body,replayed=false}){
    ...
    if(!replayed&&method!=='GET'&&method!=='HEAD'&&status>=200&&status<300){
      const eventId=randomUUID(),aggregateId=t.id||requestId,payload={actorUserId,method,path,status,auditLogId:a.rows[0]?.id||null};
    ...
    return {id:a.rows[0]?.id,actorUserId,action:`${method} ${path}`,targetType:t.type,targetId:t.id,status,requestId,replayed,metadata:detail,createdAt:a.rows[0]?.created_at||now()};
  }

...
export function createApiService(seed=[makeSampleProject()]){
  const projects=new Map(seed.map(p=>[p.id,ensureVersionStore(structuredClone(p))]));
  ...
  async function handle(req){
    ...
    try{
      if(method==='POST'&&p.length===1&&p[0]==='projects'){const id=uuid(),t=stamp(),pr=ensureVersionStore({id,name:body.name||'Untitled Project',revision:1,artifacts:[],relations:[],changeSets:[],readinessSnapshots:[],createdAt:t,updatedAt:t});projects.set(id,pr);return json(201,pr)}
      if(p[0]==='projects'&&p[1]){const project=projects.get(p[1]);if(!project)return problem(404,'PROJECT_NOT_FOUND','Project not found',p[1]);
        ...
        if(method==='GET'&&p[2]==='summary')return json(200,{id:project.id,name:project.name,revision:project.revision,artifactCount:project.artifacts.length,relationCount:project.relations.length,validation:validationSummary(project),readiness:readiness(project)});
        ...
        if(method==='POST'&&p[2]==='implementation-packages'){const pkg=buildImplementationPackage(project,body.taskIds||[],{override:!!body.readinessOverride,reason:body.reason||''});return json(201,pkg)}
      }
      ...
      if(p[0]==='change-sets'&&p[1]){const [project,cs]=findChangeSet(p[1]);if(!cs)return problem(404,'CHANGESET_NOT_FOUND','ChangeSet not found',p[1]);
        ...
        if(method==='POST'&&p[2]==='apply'){applyChangeSet(project,cs.id);return json(200,{changeSet:project.changeSets.find(c=>c.id===cs.id),revision:project.revision})}
      }

...
export class EngineeringDesignApplicationService{
  ...
  async resolveProjectId(projectId,changeSetId){if(projectId)return projectId;if(!this.repository.findProjectIdByChangeSetId)return null;return this.repository.findProjectIdByChangeSetId(changeSetId)}
  ...
  async applyChangeSet({projectId=null,changeSetId,actorUserId,traceId=null}){
    const resolved=await this.resolveProjectId(projectId,changeSetId);if(!resolved)throw Object.assign(new Error('ChangeSet project not found'),{status:404,code:'CHANGESET_NOT_FOUND'});

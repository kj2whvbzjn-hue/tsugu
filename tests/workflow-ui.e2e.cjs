...
(async()=>{
 ...
 try{
  ...
  await page.getByRole('button',{name:/受付保存仕様/}).click();await page.getByRole('heading',{name:'受付保存仕様',exact:true}).waitFor();assert.ok((await page.locator('.tsugu-spec-body').innerText()).length>3000);await page.getByRole('button',{name:'前の詳細へ戻る'}).click();await page.getByRole('heading',{name:'登録の実装',exact:true}).waitFor();

...
export function createHttpGateway({api=createApiService(),verifyBearer,auditOutbox=createAuditOutboxStore(),idempotency=createIdempotencyStore(),projectAccessRepository=null,logger=createStructuredLogger({sink:null}),metrics=createMetrics(),rateLimiter=createRateLimiter(),readinessCheck=async()=>true}={}){
  ...
  const handler=async(req,res)=>{const started=Date.now(),requestId=req.headers['x-request-id']||randomUUID(),url=new URL(req.url,'http://local'),method=(req.method||'GET').toUpperCase(),route=routeKey(url.pathname);let principal=null,body={},projectId=null,status=500,idem=null,idemStored=false;try{
    ...
    if(response.headers?.['x-transactional-audit']!=='true')await auditOutbox.record({principal,method,path:url.pathname,status,requestId,body});send(res,status,{'x-request-id':requestId,...response.headers},response.body)
  }catch(err){

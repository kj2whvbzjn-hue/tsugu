...
(async()=>{
 ...
 try{
  ...
  await page.setViewportSize({width:1440,height:1000});await page.evaluate(()=>new Promise(resolve=>requestAnimationFrame(()=>requestAnimationFrame(resolve))));await page.evaluate(()=>scrollTo(0,document.documentElement.scrollHeight));await page.evaluate(()=>new Promise(resolve=>requestAnimationFrame(()=>requestAnimationFrame(resolve))));const stickyState=await page.locator('.tabs').evaluate(el=>{const cs=getComputedStyle(el),r=el.getBoundingClientRect();return {position:cs.position,top:cs.top,rectTop:Math.round(r.top),visible:r.bottom>0&&r.top<innerHeight};});assert.equal(stickyState.position,'sticky');assert.equal(stickyState.top,'0px');assert.equal(stickyState.visible,true);const sticky=stickyState.rectTop;const desktopMetrics=await page.evaluate(()=>({scrollWidth:document.documentElement.scrollWidth,clientWidth:document.documentElement.clientWidth}));assert.ok(desktopMetrics.scrollWidth<=desktopMetrics.clientWidth+1,JSON.stringify(desktopMetrics));const desktopPersistentOccupancy=await persistentOccupancy();await page.screenshot({path:'outputs/workflow-desktop.png',fullPage:true});
  assert.deepEqual(errors,[]);result.scenarios.push('Reload restores canonical project; no browser persistence; mobile and desktop no overflow; desktop primary navigation remains sticky');result.status='PASS';result.metrics=metrics;result.desktopMetrics=desktopMetrics;result.stickyTabsTop=sticky;result.stickyTabsState=stickyState;result.visibility={persistentOccupancy:{mobile:mobilePersistentOccupancy,desktop:desktopPersistentOccupancy}};result.puts=puts;result.consoleErrors=errors;

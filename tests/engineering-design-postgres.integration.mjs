...
try{
  ...
  const client=await pool.connect();try{const audit=await client.query(`select count(*)::int as n from audit_logs where project_id=$1 and action='changeset.apply'`,[project.id]),outbox=await client.query(`select count(*)::int as n from outbox_events where aggregate_id=$1 and event_type='changeset.applied'`,[project.id]),bucket=await client.query(`select count(*)::int as n from rate_limit_buckets where bucket_key='pg-shared-user'`);assert.equal(audit.rows[0].n,1);assert.equal(outbox.rows[0].n,1);assert.equal(bucket.rows[0].n,0)}finally{client.release()}

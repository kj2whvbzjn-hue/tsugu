...
try{
  ...
  const client=await pool.connect();try{
    const transactionalAudit=await client.query(`select count(*)::int as n from audit_logs where project_id=$1 and action='changeset.apply'`,[project.id]),transactionalOutbox=await client.query(`select count(*)::int as n from outbox_events where aggregate_id=$1 and event_type='changeset.applied'`,[project.id]),patchAudit=await client.query(`select count(*)::int as n from audit_logs where project_id=$1 and action like 'PATCH /api/v1/artifacts/%'`,[project.id]),patchOutbox=await client.query(`select count(*)::int as n from outbox_events where aggregate_id=$1 and event_type='artifact.updated'`,[req.id]),replayAudit=await client.query(`select count(*)::int as n from audit_logs where project_id=$1 and action=$2 and detail->>'replayed'='true'`,[project.id,`POST /api/v1/change-sets/${changeSetId}/apply`]),bucket=await client.query(`select count(*)::int as n from rate_limit_buckets where bucket_key='pg-shared-user'`),idem=await client.query(`select state from idempotency_records where scope like $1`,[`pg-user:POST:/api/v1/change-sets/${changeSetId}/apply:%`]);
    assert.equal(transactionalAudit.rows[0].n,1);assert.equal(transactionalOutbox.rows[0].n,1);assert.equal(patchAudit.rows[0].n,1);assert.equal(patchOutbox.rows[0].n,1);assert.equal(replayAudit.rows[0].n,1);assert.equal(bucket.rows[0].n,0);assert.equal(idem.rows[0].state,'completed')

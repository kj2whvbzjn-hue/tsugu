...
async function readJson(req,{maxBytes=1_048_576}={}){const chunks=[];let total=0;for await(const c of req){total+=c.length;if(total>maxBytes)throw Object.assign(new Error(`Request body exceeds ${maxBytes} bytes`),{status:413,code:'PAYLOAD_TOO_LARGE'});chunks.push(c)}if(!chunks.length)return{};const text=Buffer.concat(chunks).toString('utf8');try{return text?JSON.parse(text):{}}catch{throw Object.assign(new Error('Malformed JSON request body'),{status:400,code:'INVALID_JSON'})}}
...
function normalizeRequestId(value){const raw=Array.isArray(value)?value[0]:value;if(typeof raw==='string'&&/^[A-Za-z0-9._:-]{1,128}$/.test(raw))return raw;return randomUUID()}

export function createHttpGateway({api=createApiService(),verifyBearer,auditOutbox=createAuditOutboxStore(),idempotency=createIdempotencyStore(),projectAccessRepository=null,logger=createStructuredLogger({sink:null}),metrics=createMetrics(),rateLimiter=createRateLimiter(),readinessCheck=async()=>true,httpOptions={}}={}){
  if(!verifyBearer)throw new Error('verifyBearer is required');const options={bodyLimitBytes:1_048_576,requestTimeoutMs:30_000,headersTimeoutMs:15_000,keepAliveTimeoutMs:5_000,...httpOptions};
  const handler=async(req,res)=>{const started=Date.now(),requestId=normalizeRequestId(req.headers['x-request-id']),url=new URL(req.url,'http://local'),method=(req.method||'GET').toUpperCase(),route=routeKey(url.pathname);let principal=null,body={},projectId=null,status=500,idem=null,idemStored=false;try{
    ...
    principal=await verifyBearer(req.headers.authorization);const rate=await rateLimiter.check(principal.userId||'anonymous');if(!rate.allowed)throw Object.assign(new Error('Rate limit exceeded'),{status:429,code:'RATE_LIMITED',rate});const permission=permissionForRequest(method,url.pathname);projectId=resolveProjectId(api,url.pathname);if(projectAccessRepository&&projectId)await authorizeProject({principal,projectId,permission,accessRepository:projectAccessRepository});else authorize(principal,permission);if(method!=='GET'&&method!=='HEAD')body=await readJson(req,{maxBytes:options.bodyLimitBytes});
    idem=idempotency.appliesTo(method,url.pathname)?await idempotency.lookup({principal,method,path:url.pathname,key:req.headers['idempotency-key'],body}):null;
    if(idem?.hit){const cached=idem.response;status=cached.status;await auditOutbox.record({principal,method,path:url.pathname,status,requestId,body,replayed:true,projectId});send(res,status,{'x-request-id':requestId,'idempotency-replayed':'true',...cached.headers},cached.body);return}
    ...
    if(idem?.reserved&&req.headers['idempotency-key']){if(status>=200&&status<300){await idempotency.store({scope:idem.scope,fingerprint:idem.fingerprint,response});idemStored=true}else{try{await idempotency.release?.(idem.scope)}catch{}idem=null}}
    if(response.headers?.['x-transactional-audit']!=='true')await auditOutbox.record({principal,method,path:url.pathname,status,requestId,body,projectId});send(res,status,{'x-request-id':requestId,...response.headers},response.body)
  }catch(err){
    if(idem?.reserved&&!idemStored){try{await idempotency.release?.(idem.scope)}catch{}}
    status=err.status||500;const code=err.code||'REQUEST_FAILED',problem={type:`https://errors.local/${String(code).toLowerCase().replaceAll('_','-')}`,title:err.message,status,code,traceId:requestId,errors:[]};try{await auditOutbox.record({principal,method,path:url.pathname,status,requestId,body,projectId})}catch{}const headers={'x-request-id':requestId};if(err.rate){headers['retry-after']=String(Math.max(1,Math.ceil((err.rate.resetAt-Date.now())/1000)));headers['x-ratelimit-remaining']='0'}send(res,status,headers,problem)
  ...
  return{handler,api,auditOutbox,idempotency,projectAccessRepository,logger,metrics,rateLimiter,httpOptions:options,listen(port=4180,host='127.0.0.1'){const server=http.createServer(handler);server.requestTimeout=options.requestTimeoutMs;server.headersTimeout=options.headersTimeoutMs;server.keepAliveTimeout=options.keepAliveTimeoutMs;return new Promise(resolve=>server.listen(port,host,()=>resolve(server)))}};
}

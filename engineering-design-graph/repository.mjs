...
export class MemoryProjectRepository{
  ...
  async findProjectIdByChangeSetId(changeSetId){for(const p of this.projects.values())if(p.changeSets?.some(x=>x.id===changeSetId))return p.id;return null}
...
export class PostgresProjectRepository{
  ...
  async findProjectIdByChangeSetId(changeSetId){const r=await this.client.query('select project_id from change_sets where id=$1',[changeSetId]);return r.rows[0]?String(r.rows[0].project_id):null}

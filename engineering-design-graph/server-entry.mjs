...
export async function startEngineeringDesignServer({env=process.env,fetchFn=fetch,pgModule=null,logger=createStructuredLogger()}={}){
  ...
  const runtime=await createEngineeringDesignRuntime({pool,verifyBearer,logger,rateLimiter,httpOptions:config.http}),server=await runtime.gateway.listen(config.port,config.host);

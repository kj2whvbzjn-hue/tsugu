...
rm -rf -- "$site_dir"
mkdir -p "$site_dir"
for asset in index.html workflow.css workflow-detail-nav.css workflow-compact-menu.css favicon.svg workflow-domain.mjs workflow-git.mjs workflow-app.mjs workflow-detail-nav.mjs workflow-compact-menu.mjs workflow-import.mjs manual.html project-import-example.json; do
  test -f "static/$asset"
